...
#6
Select Workers.W_Name as 'Worked between two defined days' from Workers, EntrepriseWorkersCalendar, Calendar where  Workers.Id_Workers=EntrepriseWorkersCalendar.Id_Workers  and Calendar.Id_Calendar=EntrepriseWorkersCalendar.Id_Calendar and (Calendar.D_Day>=21 and Calendar.D_Day<=23); 
#7
Select count(Workers.Id_Workers) as 'Number of workers between two hours' from Workers, EntrepriseWorkersCalendar, Calendar where  Workers.Id_Workers=EntrepriseWorkersCalendar.Id_Workers  and Calendar.Id_Calendar=EntrepriseWorkersCalendar.Id_Calendar  and (EntrepriseWorkersCalendar.int_time>='08:00:00'  and EntrepriseWorkersCalendar.end_time<='20:00:00') ; 
#8
